...
include_once(_PS_MODULE_DIR_.'psverticalmenu/loader.php');

class PsVerticalMenu extends Module {
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Ps Vertical Megamenu');
		$this->description = $this->l('Ps Vertical Megamenu Support Pts Framework Version 2.0');
		$this->mscript = new PtsVerMenuMcrypt();
		$this->widget = new PsVerticalMenuWidget();
	}
	...
	{
		/* Adds Module */
		if (parent::install() && $this->registerHook('leftcolumn') && $this->registerHook('header') && $this->registerHook('actionObjectLanguageAddAfter'))
		{
			$res = true;
			/* Creates tables */
			//$this->createTables();
	...
	public function getContent()
	{
		...
		if (Tools::getValue('doupdatepos') && Tools::isSubmit('updatePosition'))
		{
			...
			{
				$i = 0;
				foreach ($menus as $id_psverticalmenu)
				{
					$res &= Db::getInstance()->execute('
                        UPDATE `'._DB_PREFIX_.'psverticalmenu` SET `position` = '.(int)$i.', id_parent = '.(int)$id_parent.' 
                        WHERE `id_psverticalmenu` = '.(int)$id_psverticalmenu
					);
					$i++;
				}
			}
		...
		{
			$obj = new PsVerticalMegamenu((int)Tools::getValue('id_psverticalmenu'));
			$res = $obj->delete();
			Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'));
		}
		if (Tools::isSubmit('save'.$this->name) && Tools::isSubmit('active'))
		{
			if ($id_psverticalmenu = Tools::getValue('id_psverticalmenu'))
				$megamenu = new PsVerticalMegamenu((int)$id_psverticalmenu);
			else
				$megamenu = new PsVerticalMegamenu();
			...
			if ($megamenu->validateFields(false) && $megamenu->validateFieldsLang(false))
			{
				...
				if (isset($_FILES['image']) && isset($_FILES['image']['tmp_name']) && !empty($_FILES['image']['tmp_name']))
				{
					...
					elseif (!($tmp_name = tempnam(_PS_TMP_IMG_DIR_, 'PS')) || !move_uploaded_file($_FILES['image']['tmp_name'], $tmp_name))
						return false;
					elseif (!ImageManager::resize($tmp_name, dirname(__FILE__).'/views/img/'.$_FILES['image']['name']))
						return false;
					unlink($tmp_name);
				...
				else if (Tools::getIsset('delete_icon'))
				{
					if ($megamenu->image)
					{
						unlink(dirname(__FILE__).'/views/img/'.$megamenu->image);
						$megamenu->image = '';
						$megamenu->save();
					}
				}
				$this->clearCache();
				Tools::redirectAdmin(AdminController::$currentIndex.'&configure=psverticalmenu&save'
					.$this->name.'&token='.Tools::getValue('token').'&id_psverticalmenu='.$megamenu->id);
			}
			else
	...
	{
		$this->context->controller->addCSS($this->_path.'views/css/psverticalmenu.css', 'all');
	}
	...
	{
		$this->context->controller->addJS(__PS_BASE_URI__.'modules/psverticalmenu/views/js/admin/jquery.nestable.js');
		$this->context->controller->addJS(__PS_BASE_URI__.'modules/psverticalmenu/views/js/admin/form.js');
		...
		$this->context->controller->addCSS(__PS_BASE_URI__.'modules/psverticalmenu/views/css/admin/form.css');

		$this->widget->loadEngines();

		$id_lang = $this->context->language->id;
		$id_psverticalmenu = (int)Tools::getValue('id_psverticalmenu');
		$obj = new PsVerticalMegamenu($id_psverticalmenu);
		$tree = $obj->getTree();
		$categories = PsVerticalMenuHelper::getCategories();
		...
		$image_url = '';
		if ($obj->image && is_file(_PS_MODULE_DIR_.'psverticalmenu/views/img/'.$obj->image))
		{
			$protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? 'https://' : 'http://';
			$image_url = Tools::htmlentitiesutf8($protocol.$_SERVER['HTTP_HOST'].__PS_BASE_URI__).'modules/psverticalmenu/views/img/'.$obj->image;
		}
        ...
        ';
		$addnew = AdminController::$currentIndex.'&token='.Tools::getAdminTokenLite('AdminModules').'&configure='
				.$this->name.'&tab_module=front_office_features&module_name='.$this->name;
		$output .= '<div class="col-md-4"><div class="panel panel-default"><h3 class="panel-title">'.$this->l('Tree Megamenu Management').'</h3>
				<div class="panel-content"><div class="alert alert-warning">'.
				$this->l('To sort orders or update parent-child, you drap and drop expected menu, then click to Update button to Save').'</div><hr>
					<p>
                    <div class="pull-right">
                    <input type="button" value="'.$this->l('New Category').'" id="addcategory" data-loading-text="
                    '.$this->l('Processing ...').'" class="btn btn-default " name="addcategory">
                     <a href="'.$liveeditor_url.'" class="btn btn-danger"> '
						.$this->l('Editor').'</a> </div>
                    <input type="button" value="'.$this->l('Update Positions').'" id="serialize" data-loading-text="
                    '.$this->l('Processing ...').'" class="btn btn-primary" name="serialize"></p><hr>'.$tree.'<p>
                    <input type="button" value="'.$this->l('Update Positions').'" id="serialize-tree" 
                    data-loading-text="'.$this->l('Processing ...').'" class="btn btn-primary" name="serialize"></p></div></div></div>
		...
		$output .= '  <div class="tab-pane" id="widgets">
                ...
                <div>
                    <p><a href="'.AdminController::$currentIndex.'&configure='.$this->name.'&widgets=1&
                    token='.Tools::getAdminTokenLite('AdminModules').'" class="btn btn-info pts-modal-action btn btn-modeal btn-success btn-action">
                    '.$this->l('Create Widget').'</a></p>
                </div>
	...
	public function widgetsList()
	{
		...
		$helper->toolbar_btn['new'] = array(
			'href' => AdminController::$currentIndex.'&configure='.$this->name.'&add'.$this->name.'&token='.
				Tools::getAdminTokenLite('AdminModules').'&widgets=1',
			'desc' => $this->l('Add new')
		);
	...
	protected function ajxgenmenu()
	{
		...
		{
			Configuration::updateValue('PS_VERTICAL_MEGAMENU_PARAMS', '');
			$this->clearCache();
		}
		$params['params'] = Configuration::get('PS_VERTICAL_MEGAMENU_PARAMS');

		if (isset($params['params']) && !empty($params['params']))
			$params['params'] = Tools::jsonDecode($params['params']);

		$obj = new PsVerticalMegamenu($parent);
	...
	public function ajxmenuinfo()
	{
		if (Tools::getValue('params'))
		{
			...
			Configuration::updateValue('PS_VERTICAL_MEGAMENU_PARAMS', $params, true);
			$this->clearCache();
		}
	...
	protected function showLiveEditorSetting()
	{
		...
		$this->context->controller->addJS(__PS_BASE_URI__.'modules/psverticalmenu/views/js/admin/form.js');
		$this->context->controller->addJS(__PS_BASE_URI__.'modules/psverticalmenu/views/js/admin/liveeditor.js');

		$this->context->controller->addCss(__PS_BASE_URI__.'modules/psverticalmenu/views/css/admin/liveeditor.css');

		$this->context->controller->addCss(__PS_BASE_URI__.'modules/psverticalmenu/views/css/admin/widget.css');

		$tcss = _PS_ROOT_DIR_.'/themes/'.$this->context->shop->getTheme().'/css/modules/psverticalmenu/views/css/psverticalmenu.css';

		if (file_exists($tcss))
			$this->context->controller->addCss(_THEMES_DIR_.$this->context->shop->getTheme().'/css/modules/psverticalmenu/views/css/psverticalmenu.css');
		else
			$this->context->controller->addCss(__PS_BASE_URI__.'modules/psverticalmenu/views/css/psverticalmenu.css');
	...
	{
		$this->context->controller->addCss(__PS_BASE_URI__.'modules/psverticalmenu/views/css/admin/widget.css');
		if (Tools::isSubmit('deletepsverticalmenu') && Tools::getValue('id_widget'))
		{
			$model = new PsVerticalMenuWidget(Tools::getValue('id_widget'));
		...
		if (Tools::getValue('id_widget'))
			$model = new PsVerticalMenuWidget($id);
		else
			$model = $this->widget;
		...
		if (Tools::isSubmit('widgets') && Tools::isSubmit('savepsverticalmenu')
			&& Tools::isSubmit('widget_name') && Tools::isSubmit('widget_type')
			&& Tools::getValue('widget_type') && Tools::getValue('widget_name'))
		{
	...
	public function renderwidget()
	{
		...
		{
			unset($widgets[0]);
			$model = new PsVerticalMenuWidget();
			...
			foreach ($widgets as $wid)
			{
				...
				$output .= $this->getWidgetContent($content['type'], $content['data']);
			}
			echo $output;
		}
		die;
	...
	{
		return $this->hookDisplayTop($params, 'headerright');
	}

	public function hookDisplaySlideshow($params)
	{
		return $this->hookDisplayTop($params, 'slideshow');
	}

	public function hookDisplayMainmenu($params)
	{
		return $this->hookDisplayTop($params, 'mainmenu');
	}

	public function hookTopNavigation($params)
	{
		return $this->hookDisplayTop($params, 'topnavigation');
	}

	public function hookDisplayPromoteTop($params)
	{
		return $this->hookDisplayTop($params, 'promotetop');
	}

	public function hookDisplayBottom($params)
	{
		return $this->hookDisplayTop($params, 'bottom');
	}

	public function hookDisplayContentBottom($params)
	{
		return $this->hookDisplayTop($params, 'contentbottom');
	}

	public function hookRightColumn($params)
	{
		return $this->hookDisplayTop($params, 'right');
	}

	public function hookLeftColumn($params)
	{
		return $this->hookDisplayTop($params, 'left');
	}

	public function hookdisplayHome($params)
	{
		return $this->hookDisplayTop($params, 'home');
	}

	public function hookFooter($params)
	{
		return $this->hookDisplayTop($params, 'footer');
	}
	...
	public function hookDisplayTop($params, $hook = 'top')
	{
		...
		if (!$this->isCached($tpl, $cache_id))
		{
			...
			$obj = new PsVerticalMegamenu();
			$obj->setModule($this);
			$psverticalmenu = $obj->getFrontTree(1, false, $params['params']);

			$this->smarty->assign('psverticalmenu', $psverticalmenu);
		}
		$this->smarty->assign('hook_name', $hook);
