...
class PsVerticalMegamenu extends ObjectModel {

	public $id;
	public $id_psverticalmenu;
	public $image;
	public $icon_class;
	public $id_parent = 1;
	public $is_group = 0;
	...
	public static $definition = array(
		'table' => 'psverticalmenu',
		'primary' => 'id_psverticalmenu',
		'multilang' => true,
		'fields' => array(
	...
	{
		$this->position = PsVerticalMegamenu::getLastPosition((int)$this->id_parent);
		$this->level_depth = $this->calcLevelDepth();
		$context = Context::getContext();
		$id_shop = $context->shop->id;
		$res = parent::add($autodate, $null_values);
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'psverticalmenu_shop` (`id_shop`, `id_psverticalmenu`)
	...
	protected function recursiveDelete(&$to_delete, $id_psverticalmenu)
	{
		if (!is_array($to_delete) || !$id_psverticalmenu)
			die(Tools::displayError());

		$result = Db::getInstance()->executeS('
		SELECT `id_psverticalmenu`
		FROM `'._DB_PREFIX_.'psverticalmenu`
		WHERE `id_parent` = '.(int)$id_psverticalmenu);
		foreach ($result as $row)
		{
			$to_delete[] = (int)$row['id_psverticalmenu'];
			$this->recursiveDelete($to_delete, (int)$row['id_psverticalmenu']);
		}
	}
	...
	public function delete()
	{
		...
		Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'psverticalmenu` WHERE `id_psverticalmenu` IN ('.$list.')');
		Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'psverticalmenu_shop` WHERE `id_psverticalmenu` IN ('.$list.')');
		Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'psverticalmenu_lang` WHERE `id_psverticalmenu` IN ('.$list.')');
		PsVerticalMegamenu::cleanPositions($this->id_parent);
		return true;
	}
	...
	public function deleteSelection($menus)
	{
		...
		{
			$obj_menu = new PsVerticalMegamenu($id_psverticalmenu);
	...
	{
		$parent_bt_vertical_megamenu = new PsVerticalMegamenu($this->id_parent);
		if (!$parent_bt_vertical_megamenu)
			die('parent Menu does not exist');
		return $parent_bt_vertical_megamenu->level_depth + 1;
	}

	public function updatePosition($way, $position)
	{
		if (!$res = Db::getInstance()->executeS('
			SELECT cp.`id_psverticalmenu`, cp.`position`, cp.`id_parent`
			FROM `'._DB_PREFIX_.'psverticalmenu` cp
			WHERE cp.`id_parent` = '.(int)$this->id_parent.'
			ORDER BY cp.`position` ASC'
						))
			return false;
		foreach ($res as $menu)
			if ((int)$menu['id_psverticalmenu'] == (int)$this->id)
				$moved_menu = $menu;
		...
		return (Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'psverticalmenu`
			SET `position`= `position` '.($way ? '- 1' : '+ 1').'
			WHERE `position`
			'.($way ? '> '.(int)$moved_menu['position'].' AND `position` <= '.(int)$position
			...
			AND `id_psverticalmenu`='.(int)$moved_menu['id_psverticalmenu']));
	}
	...
	public static function cleanPositions($id_parent)
	{
		...
		for ($i = 0; $i < $sizeof; ++$i)
		{
			...
			SET `position` = '.(int)$i.'
			WHERE `id_parent` = '.(int)$id_parent.'
			AND `id_psverticalmenu` = '.(int)$result[$i]['id_psverticalmenu'];
	...
	{
		return (Db::getInstance()->getValue('SELECT MAX(position)+1 FROM `'._DB_PREFIX_.'psverticalmenu` WHERE `id_parent` = '.(int)$id_parent));
	}

	public function getInfo($id_psverticalmenu, $id_lang = null, $id_shop = null)
	{
		if (!$id_lang)
		...
		$sql = 'SELECT m.*, md.title, md.description, md.content_text, md.submenu_content_text
				...
				LEFT JOIN '._DB_PREFIX_.'psverticalmenu_lang md ON m.id_psverticalmenu = md.id_psverticalmenu AND md.id_lang = '.(int)$id_lang
						.' JOIN '._DB_PREFIX_.'psverticalmenu_shop bs ON m.id_psverticalmenu = bs.id_psverticalmenu AND bs.id_shop = '.(int)$id_shop;
		$sql .= ' WHERE m.id_psverticalmenu='.(int)$id_psverticalmenu;

		return Db::getInstance()->executeS($sql);
	}

	public function getChild($id_psverticalmenu = null, $id_lang = null, $id_shop = null, $active = false)
	{
		if (!$id_lang)
			$id_lang = Context::getContext()->language->id;
		if (!$id_shop)
		...
		$sql = ' SELECT m.*, md.title, md.text, md.description, md.content_text, md.submenu_content_text
				FROM '._DB_PREFIX_.'psverticalmenu m
				LEFT JOIN '._DB_PREFIX_.'psverticalmenu_lang md ON m.id_psverticalmenu = md.id_psverticalmenu AND md.id_lang = '.(int)$id_lang
						.' JOIN '._DB_PREFIX_.'psverticalmenu_shop bs ON m.id_psverticalmenu = bs.id_psverticalmenu AND bs.id_shop = '.(int)$id_shop;
		if ($active)
			$sql .= ' WHERE m.`active`=1 ';

		if ($id_psverticalmenu != null)
			$sql .= ' WHERE id_parent='.(int)$id_psverticalmenu;
	...
	public function genOption($parent, $level = 0, $selected, $output = array())
	{
		if ($this->hasChild($parent))
		{
			...
			{
				$selected == $menu['id_psverticalmenu'] ? 'selected="selected"' : '';
				$output[] = array('id' => $menu['id_psverticalmenu'],
					'title' => str_repeat('-', $level).' '.$menu['title'].' (ID:'.$menu['id_psverticalmenu'].')', 'selected' => $selected);
				if ($menu['id_psverticalmenu'] != $parent)
					$output = $this->genOption($menu['id_psverticalmenu'], $level + 1, $selected, $output);
			}
		}
		return $output;
	}
	...
	public function genTree($parent, $level)
	{
		if ($this->hasChild($parent))
		{
			...
			foreach ($data as $menu)
			{
				$cls = Tools::getValue('id_psverticalmenu') == $menu['id_psverticalmenu'] ? 'selected' : '';
				$output .= '<li id="list_'.$menu['id_psverticalmenu'].'" class="'.$cls.'">
				<div><span class="disclose"><span></span></span>'.($menu['title'] ? $menu['title'] : '').' 
				(ID:'.$menu['id_psverticalmenu'].') <span class="quickedit" rel="id_'.$menu['id_psverticalmenu'].'">E</span>
				<span class="quickdel" rel="id_'.$menu['id_psverticalmenu'].'">D</span></div>';

				if ($menu['id_psverticalmenu'] != $parent)
					$output .= $this->genTree($menu['id_psverticalmenu'], $level + 1);
				$output .= '</li>';
			}
			$output .= '</ol>';
			return $output;
	...
	{
		$t = sprintf($this->edit_string, $menu['id_psverticalmenu'], $menu['is_group'], $menu['colums']);
		if ($this->is_live_edit)
		{
			if (isset($menu['megaconfig']->subwidth) && $menu['megaconfig']->subwidth)
				$t .= ' data-subwidth="'.$menu['megaconfig']->subwidth.'" ';
			$t .= ' data-submenu="'.(isset($menu['megaconfig']->submenu) ? $menu['megaconfig']->submenu : $this->hasChild($menu['id_psverticalmenu'])).'"';
	...
	public function getFrontTree($parent = 1, $edit = false, $params = array())
	{
		...
		else
		{
			...
			$this->model_menu_widget = new PsVerticalMenuWidget();
		...
		foreach ($childs as $child)
		{
			...
			$child['megamenu_id'] = $child['id_psverticalmenu'];
		...
		$this->image_base_url = Tools::htmlentitiesutf8($protocol.$_SERVER['HTTP_HOST'].__PS_BASE_URI__).'modules/psverticalmenu/views/img/';
		...
		if ($this->hasChild($parent))
		{
			...
			foreach ($data as $menu)
			{
				...
				if ($this->hasChild($menu['megamenu_id']) || $menu['type_submenu'] == 'html')
				{
					...
					elseif ($menu['image'] && file_exists(_PS_MODULE_DIR_.'psverticalmenu/views/img/'.$menu['image']))
					...
					if ($menu['text'])
						$output .= '<span class="menu-desc">'.$menu['text'].'</span>';
					if ($menu['description'])
						$output .= '<span class="menu-desc">'.$menu['description'].'</span>';
					if (($menu['image'] && file_exists(_PS_MODULE_DIR_.'psverticalmenu/views/img/'.$menu['image'])) || $menu['icon_class'])
						$output .= '</span>';
		...
		if (is_object($col) && isset($col->widgets) && !$this->edit_string)
		{
			...
			if (!empty($widgets))
			{
				...
				foreach ($widgets as $wid)
				{
					...
					$output .= $this->menu_module->getWidgetContent($content['type'], $content['data']);
				}
	...
	public function getCategorie($submenu_catids, $context)
	{
		...
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT DISTINCT c.id_parent, 
            ...
            AND c.id_category IN (SELECT id_category FROM `'._DB_PREFIX_.'category_group` 
            	WHERE `id_group` IN ('.pSQL($groups).') AND id_category IN ('.$submenu_catids.'))
